...
function detectTransitions(previous, current) {
  ...
  // etter seg når feilen forsvinner. Men bare når laderen faktisk sa at feilen
  // er borte — mangler feltet, er defa_error_code null, og da vet vi ingenting.
  // Å fyre «feilen er borte» på manglende data er en usann melding om noe
  // brukeren kan ha satt opp et varsel på.
  // typeof string, ikke `!== null`: da dekkes undefined også, og betingelsen
  // sier presist det den skal — at laderen faktisk oppgav en feilkode.
  if (hasError(previous) && typeof current.defa_error_code === 'string' && !hasError(current)) {
