...
function httpRequest({ host, port, path, method, headers, body, timeout }) {
  return new Promise((resolve, reject) => {
    ...
    // Absolutt frist i tillegg til inaktivitetsfristen — se samme sted i
    // cloudcharge-client.js. En CLU som trikler ut et svar skal ikke kunne
    // holde en skriving åpen i det uendelige.
...
class CluClient {
  ...
  async _call(method, path, { body, auth = false, retried = false } = {}) {
    ...
    // Tidsavbrudd, ECONNRESET, DNS-feil og brudd midt i svaret kommer ut som
    // nakne Error-objekter uten kode. Uten denne innpakningen fant
    // localizeCluError() ingen kode å slå opp, og brukeren — også på engelsk —
    // fikk den norske Node-meldingen rett i UI-et. Det er også grunnen til at
    // errors.clu_unreachable aldri ble brukt.
    let response;
    try {
      ...
      if (error instanceof CluError) throw error;
      throw new CluError(`${method} ${path} nådde ikke fram: ${error && error.message}`, {
        code: 'unreachable',
      });
    }
  ...
  async _writeConfigNow({ mutate, isUnchanged, label, expected = null, user = null }) {
    ...
    // Brukbarheten sjekkes før sammenligningen mot referansen. Var svaret tomt
    // eller ufullstendig, leste assertMatchesExpected av en null-konfig og
    // kastet en naken TypeError i stedet for den lokaliserte
    // «ufullstendig konfigurasjon»-feilen brukeren skal se.
    assertUsable(config);
    ...
    // Skrivingen gjentas ikke ved tidsavbrudd. Nådde forespørselen fram og
    // svaret ble borte, har laderen kanskje utført endringen — da er en ny
    // skriving verre enn ingen. Brukeren må få vite at utfallet er ukjent,
    // ikke «får ikke kontakt», som ville vært direkte feil.
    try {
      ...
      if (error instanceof CluError && error.code === 'unreachable') {
        throw new CluError(`${label}: svar uteble, utfallet er ukjent`, {
